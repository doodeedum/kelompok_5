...
//  Letakkan file di: ASSET/logo.png
$asset_logo = 'ASSET/logo.png';
//
...
//  Letakkan file di: ASSET/background.jpg
$asset_bg = 'ASSET/background.jpg';
//
//  ASSET 3 — IKON mata pelajaran Bahasa Inggris
//  Ukuran tampil : 54×54 px (sidebar lingkaran), 44×44 px (title bar), 69×69 px (item sidebar)
//  Format        : PNG transparan
//  Letakkan file di: ASSET/bahasa-inggris.png
$asset_icon = 'ASSET/bahasa-inggris.png';
//
//  ASSET 4 — AVATAR pengguna (opsional)
//  Ukuran tampil : 50 × 50 px  |  Format : JPG / PNG
//  Letakkan file di: ASSET/avatar.png
$asset_avatar = 'ASSET/avatar.png';
//
// ══════════════════════════════════════════════════════════════
...
<head>
  ...
  <style>
    /* ═══════════════════════════════════════════
       CSS VARIABLES — Tema Ungu (Bahasa Inggris)
       Ubah variabel di sini untuk ganti tema
    ═══════════════════════════════════════════ */
    :root {
      --aksen:       #8E44AD;   /* ungu utama */
      --aksen-dark:  #6C3483;   /* ungu hover */
      --bg-light:    #F5EEF8;   /* ungu muda */
      --bg-yellow:   #F9F383;   /* kuning navbar Kembali */
      --bg-cream:    #FDF7C5;   /* krem lingkaran sidebar */
      --bg-kartu:    #C5DE96;   /* hijau lingkaran nomor kartu bawah */
      --black:       #000000;
      --white:       #FFFFFF;
      --nav-h:       90px;
      --sidebar-w:   322px;
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ═══════════════════════════════════════════
       BODY — gradient: #FFFFFE → #F9F383 → #FFFDD1
    ═══════════════════════════════════════════ */
    body {
      min-height: 100vh;
    ...
    /* ── ASSET 2: Background image overlay ─────────────────
       opacity 0.60, fixed, full-page
       FILE : ASSET/background.jpg  (1440×1024 px) */
    body::before {
      content: '';
      position: fixed;
      inset: 0;
    ...
    .nav-logo { width: 100%; height: 100%; object-fit: contain; }
    .nav-logo-fallback {
      display: none;
    ...
    /* Tombol Kembali — 186×62, #F9F383, radius 30, Inter 20px 700 */
    .nav-kembali {
      width: 186px;
      height: 62px;
    ...
    /* ═══════════════════════════════════════════
       SIDEBAR KIRI — 322px, putih, radius kanan 16
    ═══════════════════════════════════════════ */
    .sidebar {
      width: var(--sidebar-w);
      flex-shrink: 0;
    ...
    /* Lingkaran ikon — 71×69, #FDF7C5, radius 9999, border hitam */
    .sidebar-icon-wrap {
      width: 71px;
      height: 69px;
      flex-shrink: 0;
    ...
    /* ASSET 3 dalam lingkaran — 54×54px */
    .sidebar-icon-wrap img { width: 54px; height: 54px; object-fit: contain; }

    /* Nama pelajaran — Inter 24px 600, hitam */
    .sidebar-nama {
      font-family: 'Inter', sans-serif;
      font-size: 24px;
      font-weight: 600;
    ...
    /* Item bab — 280×66, border hitam, radius 16 */
    .sidebar-item {
      width: 280px;
      min-height: 66px;
      display: flex;
    ...
    /* ASSET 3: ikon kanan sidebar — 69×69px */
    .sidebar-item-icon {
      width: 69px;
      height: 69px;
      object-fit: contain;
    ...
    /* Kartu border hitam — border 1px black, radius 16, full width */
    .content-card {
      background: var(--white);
      border: 1px solid var(--black);
      border-radius: 16px;
    ...
    /* Accordion (kolom kanan — tersembunyi, tetap fungsional untuk navigasi) */
    ...
    /* ═══════════════════════════════════════════
       4 KARTU BAWAH — 207×353, border hitam, radius 13
       Nomor: 52×50, #C5DE96, radius 9999, border hitam
    ═══════════════════════════════════════════ */
    .bottom-cards {
      display: grid;
    ...
    /* Lingkaran nomor — 52×50, #C5DE96, pojok kiri atas */
    ...
    /* ═══════════════════════════════════════════
       NAVIGASI BAWAH — 123×41, #EAEAEA, radius 30
    ═══════════════════════════════════════════ */
    .nav-bottom {
      display: flex;
      justify-content: space-between;
...
<div class="page-body">
  ...
  <aside class="sidebar">
    <div class="sidebar-header">
      <!-- Lingkaran ikon — #FDF7C5, 71×69 -->
      <!-- ASSET 3 dalam lingkaran — 54×54px
           FILE : ASSET/bahasa-inggris.png -->
    ...
    <!-- Daftar bab — 280×66, border hitam, radius 16 -->
  ...
  <main class="content-area">
    ...
    <div class="content-outer">
      ...
      <!-- Judul materi — Inter 24px 600 + ASSET 3 44×44 -->
      ...
      <!-- Kartu border hitam — border 1px black, radius 16 -->
      <div class="content-card">

        <!-- Isi materi aktif — Inter 20px 400 -->
          ...
          <?php else: ?>
            ...
            <div class="materi-isi-block"
              ...
              <?php else: ?>
                ...
                <!-- Tombol Contoh — 204×41, border hitam, radius 30, Inter 20px 400 -->
        ...
        <!-- Kolom kanan disembunyikan (display:none via CSS) -->
        <div class="materi-kanan">
          <div class="materi-judul-kanan" id="judul-aktif"></div>
          <?php foreach ($konten_list as $idx => $k): ?>
          <div class="acc-item" id="konten-<?= $idx ?>">
...
<script>
  ...
  function navigasiKonten(arah) {
    ...
    if (next >= 0 && next < totalKonten) updateActive(next);
  }
