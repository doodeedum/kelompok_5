...
if ($session_id > 0) {
    try {
        $su = $pdo->prepare("SELECT nickname FROM users WHERE id = ?");
        $su->execute([$user_id]);
        $usr = $su->fetch();
        ...
        if ($result) {
            ...
            $upd = $pdo->prepare("
                ...
                WHERE id = ?
            ");
            $upd->execute([$total_answered, $correct_count, $wrong_count,
                           $score, $percentage, $grade, $session_id]);
        }
    } catch (Exception $e) {
        error_log("Quiz result2 error: " . $e->getMessage());
    }
}
...
unset($_SESSION['quiz_session_id'], $_SESSION['current_question'],
      $_SESSION['quiz_questions'], $_SESSION['skipped_questions'], $_SESSION['total_score']);
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Hasil Quiz Level 2 | TahooGa</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            /*
            ╔══════════════════════════════════════════════════════╗
            ║  📁 ASSET: Background Halaman Result Level 2        ║
            ║  File: assets/img/bg-result2.png                    ║
            ║  Ganti nama file sesuai yang kamu punya             ║
            ║  Letakkan di folder: assets/img/                    ║
            ╚══════════════════════════════════════════════════════╝
            */
            background-image: url('assets/img/bg.real.png');
            background-size: cover;
            background-position: center;
            background-repeat: no-repeat;
            background-color: #F9F383; /* fallback */
            display: flex;
        ...
        body::before {
            ...
            position: fixed;
            inset: 0;
            background: linear-gradient(180deg, rgba(255,255,254,.6) 0%, rgba(249,243,131,.6) 50%, rgba(255,253,209,.6) 100%);
            pointer-events: none;
            z-index: 0;
        }
        ...
        /*
        ╔══════════════════════════════════════════════════════╗
        ║  📁 ASSET: Logo / Maskot Navbar                     ║
        ║  File: assets/img/hero-mascot.png                   ║
        ║  Letakkan di folder: assets/img/                    ║
        ╚══════════════════════════════════════════════════════╝
        */
        .navbar img.logo { width: 90px; height: auto; }
        ...
        .score-big {
            ...
            line-height: 1.1;
            text-align: center;
            animation: popIn .5s cubic-bezier(.175,.885,.32,1.275) .2s both;
        }
        ...
        .stat-divider { background: #000; align-self: stretch; }
        ...
        .action-divider { background: #000; align-self: stretch; }
...
<div class="card-actions">
    <a href="play2.php?retry=1" class="action-btn">&lt;&lt; Coba Lagi</a>
    <div class="action-divider"></div>
    <?php if ($has_next_level): ?>
    <a href="play3.php" class="action-btn">Level Berikutnya &gt;&gt;</a>
    <?php else: ?>
