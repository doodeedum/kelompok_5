...
if (!isset($_SESSION['quiz_session_id'])) {
    ...
    $_SESSION['total_score'] = 0; // ← BARU: inisialisasi skor
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['question_id'])) {
    ...
    // ← BARU: kirim jawaban benar/salah ke sesi agar JS bisa baca via redirect
    // Gunakan flash session
...
// Ambil flash result dari jawaban sebelumnya
$last_correct = $_SESSION['last_answer_correct'] ?? null;
unset($_SESSION['last_answer_correct']); // hapus setelah dibaca
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Joan&family=Inter:wght@400;500;600;700&family=Itim&family=Jockey+One&display=swap" rel="stylesheet">
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body::before {
            ...
            position: fixed;
            inset: 0;
            background: linear-gradient(180deg, rgba(255,255,254,0.6) 0%, rgba(249,243,131,0.6) 50%, rgba(255,253,209,0.6) 100%);
            pointer-events: none;
            z-index: 0;
        }
        ...
        @keyframes scoreBump {
            0%   { transform: scale(1); }
            40%  { transform: scale(1.4); color: #2e7d00; }
        ...
        .option-key-circle:hover { background: #f9f383; transform: scale(1.06); }
        ...
        .option-box:hover { background: #f9f383; transform: translateX(4px); }

        .option-selected .option-key-circle { background: var(--green-btn); border-color: #4a7a00; }
        .option-selected .option-box        { background: var(--green-btn); border-color: #4a7a00; }

        /* State benar (animasi sebelum redirect) */
        ...
        .btn-jawab:hover  { background: #aacb6e; transform: translateY(-2px); }
        ...
        /* ── OVERLAY FEEDBACK (setelah jawab) ── */
        .feedback-overlay {
            display: none;
            position: fixed;
            inset: 0;
...
function playWrongSound() {
    const ctx = getAudioCtx();
    // Nada turun: La♭4 → Fa4 (minor 3rd down)
...
// ══════════════════════════════════════════════
//  FLASH SESSION RESULT (dari PHP)
// ══════════════════════════════════════════════
<?php if ($last_correct === 1): ?>
// Jawaban sebelumnya BENAR — putar efek
...
// Jawaban sebelumnya SALAH — putar efek
window.addEventListener('DOMContentLoaded', () => {
    playWrongSound();
});
<?php endif; ?>
...
// ══════════════════════════════════════════════
//  SUBMIT — animasi feedback sebelum kirim
// ══════════════════════════════════════════════
document.getElementById('quizForm').addEventListener('submit', function(e) {
    ...
    // Disable tombol agar tidak dobel kirim
    document.getElementById('submitBtn').disabled = true;
    // Resume AudioContext (wajib setelah gesture user)
    getAudioCtx().resume();
});
